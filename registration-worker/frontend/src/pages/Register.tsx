...
const DEFAULT_FORM: Record<string, any> = {
  ...
  count: 1,
  proxy: '',
  set_password_after_registration: false,
  executor_type: '',
  captcha_solver: 'auto',
  identity_provider: '',
  oauth_provider: '',
...
export default function Register() {
  ...
  const submit = async () => {
    ...
    if (form.platform === 'chatgpt' && form.set_password_after_registration) {
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              {form.platform === 'chatgpt' && (
                <label className="flex cursor-pointer items-start gap-3 rounded-lg border border-[var(--border-soft)] bg-[var(--chip-bg)] px-4 py-3">
                  <input
                    type="checkbox"
                    checked={Boolean(form.set_password_after_registration)}
                    onChange={event => set('set_password_after_registration', event.target.checked)}
                    className="mt-0.5 h-4 w-4 accent-[var(--accent)]"
                  />
                  <span>
                    <span className="block text-sm font-medium text-[var(--text-primary)]">
                      {t('register.setChatgptPasswordAfterRegistration')}
                    </span>
                    <span className="mt-1 block text-xs leading-5 text-[var(--text-muted)]">
                      {t('register.setChatgptPasswordAfterRegistrationDesc')}
                    </span>
                  </span>
                </label>
              )}

...
def save_account(account) -> 'AccountModel':
    ...
    with Session(engine) as session:
        ...
        if existing:
            preserve_chatgpt_password = False
            if str(account.platform or "").lower() == "chatgpt" and existing.password:
                overview_model = session.get(AccountOverviewModel, existing.id)
                existing_overview = overview_model.get_summary() if overview_model else {}
                incoming_extra = dict(getattr(account, "extra", {}) or {})
                incoming_overview = dict(incoming_extra.get("account_overview") or {})
                existing_status = str(existing_overview.get("password_status") or "").strip().lower()
                incoming_status = str(incoming_overview.get("password_status") or "").strip().lower()
                incoming_password = str(getattr(account, "password", "") or "")
                incoming_is_configured = incoming_status == "configured" and bool(incoming_password)
                preserve_chatgpt_password = not incoming_is_configured

                if preserve_chatgpt_password:
                    incoming_overview["password_status"] = existing_status or "unknown"
                    incoming_overview["password_source"] = str(
                        existing_overview.get("password_source") or "none"
                    ).strip().lower()
                    if existing_status == "configured":
                        password_error = incoming_overview.pop("password_error", None)
                        if password_error:
                            incoming_overview["password_last_error"] = password_error
                    incoming_extra["account_overview"] = incoming_overview
                    account.extra = incoming_extra

            if not preserve_chatgpt_password:

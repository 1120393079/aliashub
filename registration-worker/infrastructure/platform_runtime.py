...
PASSWORD_ACTION_IDS = {"set_password"}
...
PASSWORD_ACTION_PUBLIC_DATA_KEYS = {
    "password_status",
    "password_source",
...
def _sanitize_password_action_error(error: Any, params: dict[str, Any] | None = None) -> str:
    text = str(error or "").strip()
    params = dict(params or {})
    for value in sorted(
        {
            str(params.get("password") or ""),
            str(params.get("proxy") or ""),
        }
        - {""},
        key=len,
        reverse=True,
    ...
    text = re.sub(r"(https?://[^\s?]+)\?[^\s]+", r"\1?[redacted-query]", text, flags=re.I)
    text = re.sub(
...
def _password_action_public_data(
    data: Any,
    ...
    for key, value in data.items():
        if key not in PASSWORD_ACTION_PUBLIC_DATA_KEYS or value in (None, ""):
            continue
        public[key] = (
            _sanitize_password_action_error(value, params)
            if key in {"password_error", "message"}
            else value
        )
    return public
        ...
        *,
        ...
        with Session(engine) as session:
            ...
            try:
                ...
                error = str(exc)
                if command.action_id in PASSWORD_ACTION_IDS:
                    error = _sanitize_password_action_error(error, command.params)
                return ActionExecutionResult(ok=False, error=error)

            password_action_confirmed = False
            if isinstance(result.get("data"), dict):
                data = result["data"]
                needs_save = False
                action_ok = bool(result.get("ok"))
                is_password_action = command.action_id in PASSWORD_ACTION_IDS
                password_confirmed = bool(
                    is_password_action
                    and action_ok
                    and str(data.get("password") or "")
                    and data.get("password_status") == "configured"
                    and data.get("password_source") == "settings"
                    and data.get("password_verification")
                    in {"success_url", "password_login_reconciled"}
                )
                password_action_confirmed = password_confirmed
                cancel_after_action = bool(callable(cancel_check) and cancel_check())
                if is_password_action and cancel_after_action and not password_confirmed:
                    return ActionExecutionResult(ok=False, error="任务已取消")
                if password_confirmed and cancel_after_action:
                    warning = "密码已设置，迟到取消请求未回滚已生效密码"
                    message = str(data.get("message") or "").strip()
                    if warning not in message:
                        data["message"] = "；".join(
                            item for item in (message, warning) if item
                        )
                credential_updates = {}
                if action_ok and (not is_password_action or password_confirmed):
                    credential_updates = {
                        key: value
                        for key, value in data.items()
                ...
                if password_confirmed:
                    model.password = str(data["password"])
                    ...
                    needs_save = True
                elif is_password_action:
                    existing_overview = dict((account.extra or {}).get("account_overview") or {})
                    existing_configured = bool(
                        str(model.password or "")
                        and existing_overview.get("password_status") == "configured"
                    )
                    if not existing_configured:
                        failure_error = _sanitize_password_action_error(
                            data.get("password_error") or result.get("error") or "密码设置失败",
                            command.params,
                        )
                        summary_updates.update(
                            {
                ...
                if credential_updates:
                    needs_save = True
                if needs_save:
                    cancel_before_commit = bool(callable(cancel_check) and cancel_check())
                    if cancel_before_commit and not password_confirmed:
                        return ActionExecutionResult(ok=False, error="任务已取消")
                    if password_confirmed and cancel_before_commit:
                        warning = "密码已设置，迟到取消请求未回滚已生效密码"
                        message = str(data.get("message") or "").strip()
                        if warning not in message:
                            data["message"] = "；".join(
            ...
            if command.action_id in PASSWORD_ACTION_IDS:
                result_data = _password_action_public_data(result_data, command.params)
                result_error = _sanitize_password_action_error(result_error, command.params)
                result_ok = bool(result_ok and password_action_confirmed)
                if not result_ok and not result_error:
                    result_error = "密码设置结果未通过完整校验"

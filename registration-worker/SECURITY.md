...
- **customer_portal_api（独立门户）**：生产环境必须
  - 设置独立的 `PORTAL_JWT_SECRET`（至少 32 个字符）
  - 设置独立的 `PORTAL_ADMIN_PASSWORD`（至少 12 个字符）
  - 将 `PORTAL_CORS_ORIGINS` 配置为具体可信域名
...
本项目依照 `AGPL-3.0-only` 提供。使用者需要自行确认具体部署和操作符合适用法律及目标平台服务条款；许可权利和义务以 `LICENSE` 为准。

...
FrciblyK12 是一个以 ChatGPT free 账号注册、管理和本地配置为主的 Web 面板。当前公开前端侧栏只暴露三个顶层入口：`总览`、`chatgpt free`、`设置`。

当前维护仓库：`https://github.com/sky3100/FrciblyK12`

联系与教程频道：`https://t.me/laogou_365`
...
```bash
cp .env.example .env
# Set APP_PASSWORD and VNC_PASSWORD in .env before any public deployment.
...
```env
APP_PASSWORD=<set-a-strong-random-secret>
VNC_PASSWORD=<set-a-different-strong-random-secret>
...
- Telegram：<https://t.me/laogou_365>

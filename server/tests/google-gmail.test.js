...
test("does not embed Google OAuth credentials and requires installer configuration", async () => {
  ...
  try {
    assert.deepEqual(gmail.configuration(), {
      ...
      google_oauth_client_mode: "unconfigured",
      google_oauth_client: "未配置 Google OAuth 客户端",
    });
...
test("requires a matching Google client secret when only a client ID is configured", async () => {
  const current = fixture();
  const gmail = new GoogleGmailClient({ db: current.db, encryptionKey: "test-key" });
  try {
...
test("completes configured Google OAuth and stores an encrypted refresh token", async () => {
  const current = fixture();
  const calls = [];

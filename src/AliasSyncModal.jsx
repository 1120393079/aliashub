...
export default function AliasSyncModal({ account, onClose, onSynced }) {
  ...
  const openOfficial = async () => {
    ...
    try {
      if (isIcloud) {
        if (popup) {
          popup.location.href = "https://www.icloud.com/icloudplus/";
          popup.focus();
        } else {
          setMessage("浏览器拦截了 iCloud+ 隐藏邮箱窗口，请允许此网站打开弹窗");
        }
        return;
      }
  ...
  const footer = <>
    ...
    <Button icon={ExternalLink} loading={opening} onClick={openOfficial}>{isIcloud ? "打开 iCloud+ 隐藏邮箱" : "微软官网创建"}</Button>
  ...
  return (
    <Modal
      ...
      description={account ? (isIcloud
        ? `${account.email} · 从 iCloud+ 导入已创建的邮箱别名或隐藏邮箱地址`
    ...
    >
      <div className="form-stack alias-sync-form">
        ...
        <div className="provider-login-note"><AtSign size={22} /><span><b>{account?.email}</b><small>{isIcloud ? "支持普通邮箱别名和隐藏邮箱；隐藏邮箱在 iCloud+ 创建，本系统只保存本地映射" : "只登记到 AliasHub，不会在微软官网创建或删除别名"}</small></span></div>

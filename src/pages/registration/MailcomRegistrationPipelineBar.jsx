...
export default function MailcomRegistrationPipelineBar({
  ...
  const preparation = useMemo(() => {
    ...
    const prepared = primaries.filter((item) => !String(item?.stage || "").startsWith("prepare_")).length;
    return { total: primaries.length, prepared };
  }, [pipeline?.items]);
  const preparationActive = pipelineActive && preparation.total > 0 && preparation.prepared < preparation.total;
      ...
      {pipeline && <div className="mc-pipeline-activity" aria-live="polite">
        <div className="mc-pipeline-activity-heading">
          ...
          <span><b>{pipelineActive ? "五阶段流水线并行处理中" : stageLabel(pipeline.stage || status)}</b><small>{preparationActive
            ? `每个母号准备完成后立即进入注册、提链和协议授权；母号准备 ${preparation.prepared} / ${preparation.total}。${activityMessage}`
        ...
        <small className="mc-pipeline-count-note">等待 / 进行 / 重试表示当前地址槽；成功 / 失败表示累计尝试。注册成功会立即提链，提链成功会立即自动协议授权。</small>
        <div className="mc-pipeline-stats">
          {preparation.total > 0 && <span><MailPlus size={13} />母号准备 <b>{preparation.prepared} / {preparation.total}</b></span>}
          ...
          <span><AlertTriangle size={13} />失败 <b>{failureCount}</b></span>
        ...
        {!collapsed && recentErrors.length > 0 && <div className="mc-pipeline-failures" role="alert">
          ...
          {failureCount > recentErrors.length && <small>累计失败 {failureCount} 次，此处仅显示最近 {recentErrors.length} 条。</small>}
        </div>}
      </div>}
    </section>

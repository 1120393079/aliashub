...
export default function AddressesPage({ refreshKey, onDataChange, onNavigate, initialAccountId, initialKind, initialStrategy }) {
  ...
  const registrationFailureLabel = (item) => Number(item?.registration_failure_count) > 1
    ...
    : "注册失败";
  const kindItems = [{ value: "all", label: "全部" }, { value: "primary", label: "源头号" }, ...(supportsAddressAliases ? [{ value: "official", label: selectedAccount?.provider === "icloud" ? "iCloud 地址" : selectedAccount?.provider === "mailcom" ? "mail.com 官方别名" : "官方别名" }] : []), ...(selectedAccount?.provider === "icloud" ? [{ value: "hidden", label: "隐藏邮箱" }] : []), { value: "split", label: "分裂地址" }];
  ...
  const remove = async (item) => {
    try {
      if (isImportedIcloudAddress(item) || isMailcomAlias(item)) {
        await api(`/api/addresses/${item.id}`, { method: "DELETE" });
        toast(isMailcomAlias(item) ? "mail.com 本地别名已移除" : "iCloud 本地地址映射已移除");
      } else {

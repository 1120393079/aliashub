...
import { accountOptionLabel, accountSupportsOfficialAliases, providerMeta } from "../providers.js";
...
export default function AddressesPage({ refreshKey, onDataChange, onNavigate, initialAccountId }) {
  ...
  const kindItems = [{ value: "all", label: "全部" }, { value: "primary", label: "源头号" }, ...(supportsOfficial ? [{ value: "official", label: "官方别名" }] : []), { value: "split", label: "分裂地址" }];
  useEffect(() => { if (!supportsOfficial && kind === "official") setKind("all"); }, [supportsOfficial, kind]);
  const copy = async (address) => { await copyText(address); toast("地址已复制"); };
  const remove = async (item) => {
    try { await api("/api/addresses/bulk-delete", { method: "POST", body: { ids: [item.id], accountId } }); toast("分裂地址已删除"); load(); onDataChange(); }
    catch (error) { toast(error.message, "error"); }
  };
  const toggle = (id) => setSelected((current) => current.includes(id) ? current.filter((item) => item !== id) : [...current, id]);
  const splitItems = data?.items.filter((item) => item.kind === "split") || [];
  const allPageSplitsSelected = splitItems.length > 0 && splitItems.every((item) => selected.includes(item.id));
  const togglePage = () => setSelected((current) => allPageSplitsSelected
    ? current.filter((id) => !splitItems.some((item) => item.id === id))
    : [...new Set([...current, ...splitItems.map((item) => item.id)])]);
  const bulkDelete = async () => {
    ...
    try {
      ...
      toast(result.deleted ? `已删除 ${result.deleted} 个分裂地址` : "没有需要删除的分裂地址");
  ...
  return (
    <div className="page-stack addresses-page">
      <div className="page-toolbar"><Segmented value={kind} onChange={setKind} ariaLabel="地址类型" items={kindItems} /><div className="toolbar-actions"><label className="search-box"><Search size={16} /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="搜索邮箱地址、标签或用途" /></label><select className="compact-select" value={accountId} onChange={(event) => setAccountId(event.target.value)}><option value="all">全部源头邮箱</option>{accounts.map((account) => <option key={account.id} value={account.id}>{accountOptionLabel(account)}</option>)}</select><Button icon={Download} onClick={() => { window.location.href = exportHref; }}>导出 CSV</Button><Button variant="danger-ghost" icon={Trash2} onClick={() => setDeleteMode("all")}>清空分裂地址</Button></div></div>
      {selected.length > 0 && <div className="selection-bar"><span>已选择 <b>{selected.length}</b> 个分裂地址</span><Button variant="danger" size="sm" icon={Trash2} onClick={() => setDeleteMode("selected")}>删除所选</Button></div>}
      <section className="table-panel">
        {!data ? <LoadingBlock rows={8} /> : data.items.length ? <><div className="data-table-wrap"><table className="data-table address-table"><thead><tr><th className="select-column"><input type="checkbox" aria-label="选择当前页分裂地址" checked={allPageSplitsSelected} disabled={!splitItems.length} onChange={togglePage} /></th><th>邮箱地址</th><th>类型</th><th>基础地址</th><th>源头邮箱</th><th>标签 / 用途</th><th>创建时间</th><th aria-label="操作" /></tr></thead><tbody>{data.items.map((item) => <tr className={selected.includes(item.id) ? "selected-row" : ""} key={item.id}><td className="select-column">{item.kind === "split" && <input type="checkbox" aria-label={`选择 ${item.address}`} checked={selected.includes(item.id)} onChange={() => toggle(item.id)} />}</td><td><div className="address-main"><span className={`address-kind-icon kind-${item.kind}`}>{item.kind === "primary" ? <Mail size={15} /> : item.kind === "official" ? <AtSign size={15} /> : <WandSparkles size={15} />}</span><button onClick={() => copy(item.address)}>{item.address}</button>{item.status === "disabled" && <StatusBadge status="paused">已停用</StatusBadge>}</div></td><td><span className={`kind-label kind-label-${item.kind}`}>{kindText[item.kind]}</span></td><td><span className="base-link">{item.kind === "split" ? <><Link2 size={13} />{item.parent_address}</> : "自身"}</span></td><td><span className="provider-cell"><ProviderMark provider={itemProvider(item).id} size={20} /><span><b>{item.source_email}</b><small>{itemProvider(item).name}</small></span></span></td><td><div className="label-purpose"><b>{item.label || "未命名"}</b><small>{item.purpose || "暂无用途"}</small></div></td><td><span className="muted-cell">{formatDate(item.created_at)}</span></td><td><div className="row-actions"><IconButton icon={Copy} label="复制地址" size={30} onClick={() => copy(item.address)} /><IconButton icon={Edit3} label="编辑地址" size={30} onClick={() => setEditing(item)} />{item.kind === "split" && <IconButton icon={Trash2} label="删除分裂地址" size={30} onClick={() => remove(item)} />}</div></td></tr>)}</tbody></table></div><div className="mobile-address-list">{data.items.map((item) => <article className={selected.includes(item.id) ? "selected" : ""} key={item.id}><header><span className={`address-kind-icon kind-${item.kind}`}>{item.kind === "primary" ? <Mail size={15} /> : item.kind === "official" ? <AtSign size={15} /> : <WandSparkles size={15} />}</span><span><b>{kindText[item.kind]}</b><small>{itemProvider(item).name} · {item.source_email}</small></span>{item.kind === "split" && <input type="checkbox" aria-label={`选择 ${item.address}`} checked={selected.includes(item.id)} onChange={() => toggle(item.id)} />}</header><button onClick={() => copy(item.address)}>{item.address}<Copy size={14} /></button>{item.parent_address && <p>基础：{item.parent_address}</p>}<footer><span>{item.label || "未命名"}</span><span className="row-actions"><IconButton icon={Edit3} label="编辑" onClick={() => setEditing(item)} />{item.kind === "split" && <IconButton icon={Trash2} label="删除" onClick={() => remove(item)} />}</span></footer></article>)}</div><div className="table-footer"><span>共 {data.total} 个地址</span><Pagination page={data.page} pages={data.pages} onChange={setPage} /></div></> : <EmptyState icon={AtSign} title="没有匹配的邮箱地址" description={supportsOfficial ? "在别名工厂生成官方别名或 Plus 分裂地址。" : "在别名工厂为 Google 主地址生成 Plus 分裂地址。"} action={<Button variant="primary" onClick={() => onNavigate("factory", selectedAccount ? { accountId: selectedAccount.id, mode: "split" } : {})}>进入别名工厂</Button>} />}
      </section>
      <EditAddressModal item={editing} onClose={() => setEditing(null)} onSaved={() => { load(); onDataChange(); }} />
      <ConfirmDialog open={Boolean(deleteMode)} onClose={() => setDeleteMode(null)} onConfirm={bulkDelete} loading={deleting} danger title={deleteMode === "all" ? "清空分裂地址？" : "删除所选分裂地址？"} description={deleteMode === "all" ? (accountId === "all" ? "将删除全部源头邮箱中的所有分裂地址，源头号和官方别名（如有）会保留。" : "将删除当前源头邮箱中的所有分裂地址，源头号和官方别名（如有）会保留。") : `将删除选中的 ${selected.length} 个分裂地址。`} confirmText={deleteMode === "all" ? "确认清空" : "确认删除"} />
    </div>
  );
}

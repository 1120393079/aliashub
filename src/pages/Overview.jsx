        ...
        <article className="panel accounts-summary-panel">
          ...
          {overview.recentAccounts.length ? <div className="source-summary-list">{overview.recentAccounts.map((account) => {
            ...
            return <button key={account.id} className="source-summary-row" onClick={() => onNavigate(isMailcom ? "sources" : canGenerateAliases ? "factory" : "inbox", { accountId: account.id, mode: supportsOfficial ? undefined : "split" })}>
              ...
              <span className="source-alias-mini"><b>{isMailcom ? `${account.official_used || 1}/${account.official_limit || 10}` : supportsOfficial ? `${account.official_used}/${account.official_limit}` : supportsPlus ? "Plus" : "IMAP"}</b><small>{isMailcom ? "母号 / 别名" : supportsOfficial ? "基础地址" : supportsPlus ? "分裂可用" : "收件扫描"}</small></span>
              <span className="source-alias-mini"><b>{isMailcom ? account.mailcom_aliases ?? account.official_aliases ?? 0 : supportsPlus ? account.split_count : "-"}</b><small>{isMailcom ? "官方别名" : supportsPlus ? "分裂" : "只读"}</small></span>

      ...
      <section className="source-grid">
        {!data ? <LoadingBlock rows={7} /> : items.length ? items.map((account) => {
          ...
          return <article className={`source-card source-card-${accountMeta.id}`} key={account.id}>
            ...
            <dl className="source-stats"><div><dt>{supportsImported ? "别名 / 反代" : "官方别名"}</dt><dd>{supportsAliases ? account.official_aliases : "不支持"}</dd></div><div><dt>分裂地址</dt><dd>{supportsPlus ? account.split_count : "不支持"}</dd></div><div><dt>收件扫描</dt><dd>{relativeTime(account.last_inbox_scan_at)}</dd></div><div><dt>{supportsOfficial ? "别名同步" : supportsImported ? "本地登记" : accountMeta.connectionLabel}</dt><dd>{supportsOfficial ? relativeTime(account.last_synced_at) : supportsImported ? `${account.official_aliases || 0} 个可直接注册` : account.connection_connected ? "已连接" : "待连接"}</dd></div></dl>
